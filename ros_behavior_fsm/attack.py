...
class AttackNode(Node):
    ...
    def __init__(self):
        ...
        # state
        self.active = False # true iff this node in the state machine is active. when inactive, we dont send velocity commands or compute targets.
        ...
        self.declare_parameter('attack_steering_gain', 1.0) # m/s/rad, how hard to turn to get a target in front of us
    ...
    def on_detected_objects(self, msg: PointCloud2):
        ...
        if self.last_target_pos_odom is None:
            # we've just started atatcking, so we don't have a last target to compare to
            ...
            # project target position from base link into odom so it can be saved over time
            ...
            target_pos_odom = np.array([target_pos_odom_ps.point.x, target_pos_odom_ps.point.y, 0])
            self.last_target_pos_odom = target_pos_odom
            # note the time we acquire a target so we can timeout if needed
            self.last_time_with_target = self.get_clock().now()
            # self.attack_target_dbg_publisher.publish(target_pos_odom_ps)
            ...
            # transform the point cloud into odom frame so we can compare to target pos
            xform = self.tf_buffer.lookup_transform("odom", "base_link", self.get_clock().now()-Duration(seconds=0.2))
            ...
            self.last_target_pos_odom = target_pos_odom
            self.last_time_with_target = self.get_clock().now()
            # transform new target pos to base link so we can nav to it
